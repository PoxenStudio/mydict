...
- 页面布局为两列栅格：左列固定 240px 放检索范围（`position: sticky`，往下读词条时保持可见），右列上方是搜索框、下方是结果列表。检索范围只对登录用户显示（列出其可用词典）；访客（开放使用）没有这一列，页面回落为单列。搜索框左侧不留任何说明文字，站名、搜索提示语与部署版本号统一收在页面底部的页脚（`.site-footer`），让首屏只有"搜索框 + 结果"两件事。
